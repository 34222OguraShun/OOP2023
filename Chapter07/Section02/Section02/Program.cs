...
namespace Section02 {
    class Program {
        static void Main(string[] args) {
            ...
            var registration = new Dictionary<string, List<CityInfo>>();
            ...
            while (true) {
                ...
                if (!registration.ContainsKey(pref)) {
                    registration[pref] = new List<CityInfo>();//既に県名が未登録ならリスト作成
                }
                registration[pref].Add(cityInfo);
            ...
            //一覧表示
                foreach (var Date in registration){
                    foreach (var cityDate in Date.Value) {
                        Console.WriteLine("{0} 【{1}(人口：{2}人】", Date.Key, cityDate.City, cityDate.Population);
                    }
                }
            ...
            //県名を指定しての出力処理
                ...
                foreach (var citydate in registration[input]) {
                    Console.WriteLine("【{0}(人口：{1}人)", input, citydate.City,citydate.Population);
                }
            }
